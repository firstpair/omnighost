...
// src/versioning/publication-provenance.ts
var OMNIGHOST_REPOSITORY_URL = "https://github.com/firstpair/omnighost";
var PUBLICATION_PROVENANCE_SCHEMA = "omnighost-publication-v1";
var MANAGED_PUBLICATION_SCHEMA = "omnighost-managed-publication-v1";
var PROVENANCE_LINK_TITLE = "omnighost-provenance-v1";
var HIDDEN_BLOCK_START = "<!-- omnighost-provenance:v1:start -->";
var HIDDEN_BLOCK_END = "<!-- omnighost-provenance:v1:end -->";
var SHA256_PATTERN = /^[0-9a-f]{64}$/;
var GIT_COMMIT_PATTERN = /^(?:[0-9a-f]{40}|[0-9a-f]{64})$/;
...
function canonicalManagedPublicationJson(publication) {
  var _a, _b;
  const lexicalDocument = parseLexicalDocument(publication.lexical);
  stripTrailingProvenanceNode(lexicalDocument);
  const scheduledAt = publication.status === "scheduled" ? normalizeScheduledTimestamp(publication.published_at) : null;
  return stableJsonStringify({
    ...
    custom_excerpt: (_a = publication.custom_excerpt) != null ? _a : null,
    feature_image: (_b = publication.feature_image) != null ? _b : null,
    tags: publication.tags.map((tag) => typeof tag === "string" ? tag : tag.name),
    published_at: scheduledAt
  });
}
async function hashManagedPublication(publication) {
  return sha256Hex(canonicalManagedPublicationJson(publication));
}
function createPublicationProvenance(publicationSha256, version = {}) {
...
function stripTrailingPublicationProvenance(lexical) {
  const document2 = parseLexicalDocument(lexical);
  const visible = stripTrailingProvenanceNode(document2);
  if (!visible) {
    return { lexical, removed: false };
  }
  return { lexical: JSON.stringify(document2), removed: true, visible };
}
function extractTrailingVisiblePublicationProvenance(lexical) {
  const document2 = parseLexicalDocument(lexical);
...
function publicationLexicalDocumentsEqual(left, right) {
  ...
  if (!lexicalDocumentsEqual(leftStripped.lexical, rightStripped.lexical))
    return false;
...
function extractHiddenPublicationProvenance(codeInjectionHead) {
  var _a;
  const source = codeInjectionHead != null ? codeInjectionHead : "";
  const range = findOwnedBlockRanges(source)[0];
  if (!range)
    return null;
  const values = parseOwnedMetaTags(source.slice(range.start, range.end));
  if (values.get("omnighost-schema") !== PUBLICATION_PROVENANCE_SCHEMA)
    return null;
  if (values.get("omnighost-repository") !== OMNIGHOST_REPOSITORY_URL)
    return null;
  const digest = ((_a = values.get("omnighost-publication-sha256")) != null ? _a : "").toLowerCase();
  if (!SHA256_PATTERN.test(digest))
    return null;
  const gitCommitValue = values.get("omnighost-git-commit");
  const gitCommit = gitCommitValue ? normalizeGitCommit(gitCommitValue) : void 0;
  if (gitCommitValue && !gitCommit)
    return null;
  return {
    schema: PUBLICATION_PROVENANCE_SCHEMA,
    publicationSha256: digest,
...
function selectPublicationVersion(desired, currentCodeInjectionHead, options = {}) {
  var _a, _b, _c;
  const current = extractHiddenPublicationProvenance(currentCodeInjectionHead);
  const displayedCommit = (_a = options.currentVisible) == null ? void 0 : _a.gitCommitDisplay;
  const visibleAgrees = !displayedCommit || ((_b = current == null ? void 0 : current.gitCommit) == null ? void 0 : _b.startsWith(displayedCommit)) === true;
  const currentCommitIsAllowed = !desired.gitCommit || (current == null ? void 0 : current.gitCommit) === desired.gitCommit || (current == null ? void 0 : current.gitCommit) === normalizeGitCommit(options.allowedExistingGitCommit);
  const retainCurrent = (current == null ? void 0 : current.publicationSha256) === desired.publicationSha256 && visibleAgrees && currentCommitIsAllowed;
  const gitCommit = retainCurrent ? (_c = current.gitCommit) != null ? _c : desired.gitCommit : desired.gitCommit;
  return gitCommit ? { gitCommit } : {};
}
function mergeHiddenPublicationProvenance(codeInjectionHead, provenance) {
  const source = codeInjectionHead != null ? codeInjectionHead : "";
...
function storedPublicationProvenanceMatches(currentLexical, currentCodeInjectionHead, desiredLexical, desiredCodeInjectionHead) {
  ...
  if (!desiredProvenance || !currentProvenance)
    return false;
  if (stableJsonStringify(desiredProvenance) !== stableJsonStringify(currentProvenance))
    return false;
  const desiredHead = mergeHiddenPublicationProvenance(currentCodeInjectionHead, desiredProvenance);
  if ((currentCodeInjectionHead != null ? currentCodeInjectionHead : "") !== desiredHead)
    return false;
...
async function compareManagedPublicationState(options) {
  var _a, _b;
  ...
  const embeddedSha256 = (_a = embedded == null ? void 0 : embedded.publicationSha256) != null ? _a : null;
  ...
  const visibleProvenanceMatches = stableJsonStringify(desiredVisible) === stableJsonStringify(currentVisible);
  const desiredCodeInjectionHead = mergeHiddenPublicationProvenance(
    options.currentCodeInjectionHead,
    desired.provenance
  );
  const currentCodeInjectionHead = (_b = options.currentCodeInjectionHead) != null ? _b : "";
...
function canonicalizeJson(value, insideArray) {
  ...
  if (Array.isArray(value)) {
    return value.map((entry) => {
      var _a;
      return (_a = canonicalizeJson(entry, true)) != null ? _a : null;
    });
  }
  if (!isRecord(value)) {
...
function parseLexicalDocument(lexical) {
  let parsed;
  try {
...
function lexicalChildren(document2) {
...
function stripTrailingProvenanceNode(document2) {
  const children = lexicalChildren(document2);
  if (children.length === 0)
    return null;
...
function classifyProvenanceParagraph(node) {
  ...
  if (links.length !== 1 || !isOmnighostRepositoryLink(links[0]))
    return null;
...
function collectLinkNodes(children) {
  const links = [];
  for (const child of children) {
    ...
    if (Array.isArray(child.children))
      links.push(...collectLinkNodes(child.children));
  }
  return links;
}
function isOmnighostRepositoryLink(child) {
  if (child.url !== OMNIGHOST_REPOSITORY_URL || !Array.isArray(child.children))
    return false;
  const linkText = child.children.map(collectLexicalText).join("");
  return linkText === "omnighost" && (child.title === PROVENANCE_LINK_TITLE || child.title === null || child.title === void 0);
}
function collectLexicalText(node) {
  if (!isRecord(node))
    return "";
...
function lexicalDocumentsEqual(left, right) {
  try {
    return stableJsonStringify(JSON.parse(left)) === stableJsonStringify(JSON.parse(right));
  } catch (e) {
    return false;
  }
}
...
function normalizeGitCommit(value) {
  var _a;
  const commit = (_a = normalizeOptionalString(value)) == null ? void 0 : _a.toLowerCase();
  return commit && GIT_COMMIT_PATTERN.test(commit) ? commit : void 0;
}
...
function decodeRenderedHtmlText(value) {
  return decodeHtmlAttribute(value).replace(/&nbsp;|&#160;|&#xA0;/gi, " ").replace(/&middot;|&#183;|&#xB7;/gi, "\xB7").replace(/\s+/g, " ").trim();
}
function parseOwnedMetaTags(block) {
...
function findOwnedBlockRanges(source) {
  const ranges = [];
  let cursor = 0;
...
var GhostAPIClient = class {
  postMatchesUpdate(current, requested) {
    ...
    if (current.title !== requested.title)
      return false;
    if (!publicationLexicalDocumentsEqual(current.lexical, requested.lexical))
      return false;
    ...
    if (((_b = current.feature_image) != null ? _b : null) !== requested.feature_image)
      return false;
    if (((_c = current.codeinjection_head) != null ? _c : null) !== requested.codeinjection_head)
      return false;
    if (requested.published_at !== void 0 && !timestampsEqual(current.published_at, requested.published_at))
      return false;
    const currentNames = ((_d = current.tags) != null ? _d : []).map((tag) => tag.name);
...
`);
   ...
   */
  async updatePost(postId, post, options = { visibility: "visible-hash", verifyRemoteContent: true }) {
    try {
...
async function processPostImages(app, ghostClient, markdown, file, swallowCover, cache, expectedAssetSha256) {
  var _a;
...
var SyncEngine = class {
  ...
  async seedNoteFromGhostBySlug(file, slug) {
    ...
    const bodyMarkdown = htmlToMarkdown(stripRenderedPublicationProvenanceHtml((_f = post.html) != null ? _f : ""));
...
${bodyMarkdown}`;
   ...
   */
  async syncFileToGhost(file, noteVersion, sourceContent, expectedAssetSha256) {
    var _a, _b, _c, _d, _e, _f;
    this.lastSyncedPost = null;
    try {
      const hasFrozenSource = sourceContent !== void 0;
      const content = sourceContent != null ? sourceContent : await this.app.vault.read(file);
      let diskFm = {};
      ...
      if (!hasFrozenSource) {
        ...
        const cacheFm = (_a = cache == null ? void 0 : cache.frontmatter) != null ? _a : {};
...
// src/versioning/textpack-source.ts
var TEXTPACK_PROVENANCE_SCHEMA = "omnighost-textpack-v1";
var SOURCE_KIND_SUFFIX = "source_kind";
var SOURCE_GIT_COMMIT_SUFFIX = "source_git_commit";
var SOURCE_PAYLOAD_SHA_SUFFIX = "source_payload_sha256";
var SOURCE_MARKDOWN_SHA_SUFFIX = "source_markdown_sha256";
var SOURCE_ASSETS_SUFFIX = "source_assets";
var SOURCE_SNAPSHOT_SHA_SUFFIX = "source_snapshot_sha256";
var SHA256_PATTERN2 = /^[0-9a-f]{64}$/;
var FULL_GIT_COMMIT_PATTERN = /^(?:[0-9a-f]{40}|[0-9a-f]{64})$/;
async function createTextpackSourceVersion(markdown, assets, publishing, gitCommit) {
  ...
  const assetManifest = await Promise.all(
    Array.from(assets.entries()).sort(([left], [right]) => compareUnicodeCodePoints(left, right)).map(async ([name, data]) => ({ name, sha256: await sha256Bytes(data) }))
  );
  const payloadSha256 = await sha256Bytes(new TextEncoder().encode(stableJsonStringify({
    ...
    gitCommit: normalizedCommit != null ? normalizedCommit : null
  })));
  return {
    schema: TEXTPACK_PROVENANCE_SCHEMA,
    payloadSha256,
...
function importedTextpackSourceFields(prefix, source, assets) {
  return {
...
async function hashImportedTextpackSnapshot(content, frontmatter, prefix) {
  var _a, _b;
  const authorialFrontmatter = {};
  ...
  const body = ((_b = (_a = splitFrontmatter(content)) == null ? void 0 : _a.body) != null ? _b : content).replace(/\r\n/g, "\n").replace(/\n+$/, "");
  return sha256Bytes(new TextEncoder().encode(stableJsonStringify({
    frontmatter: authorialFrontmatter,
    body
  })));
}
function importedTextpackSnapshotField(prefix, digest) {
...
function normalizeFullGitCommit(value) {
  if (typeof value !== "string")
    return void 0;
  const normalized = value.trim().toLowerCase();
  return FULL_GIT_COMMIT_PATTERN.test(normalized) ? normalized : void 0;
}
function normalizeSha256(value) {
  if (typeof value !== "string")
    return void 0;
  const normalized = value.trim().toLowerCase();
  return SHA256_PATTERN2.test(normalized) ? normalized : void 0;
}
...
function normalizedPublishingMetadata(metadata) {
  return {
...
function normalizeImportedAssets(assets) {
  return assets.map(({ path, sha256 }) => {
    var _a;
    return { path: normalizeRelativeAssetPath(path), sha256: (_a = normalizeSha256(sha256)) != null ? _a : "" };
  }).filter(({ path, sha256 }) => path !== "" && sha256 !== "").sort((left, right) => compareUnicodeCodePoints(left.path, right.path));
}
function compareUnicodeCodePoints(left, right) {
  ...
  while (true) {
    ...
    const leftPoint = (_a = leftPart.value.codePointAt(0)) != null ? _a : 0;
    const rightPoint = (_b = rightPart.value.codePointAt(0)) != null ? _b : 0;
    if (leftPoint !== rightPoint)
      return leftPoint < rightPoint ? -1 : 1;
  }
}
function parseImportedAssets(value) {
  if (typeof value !== "string")
    return null;
  let parsed;
  try {
...
function isOperationalFrontmatterKey(key, prefix) {
  if (!key.startsWith(prefix))
    return false;
  const suffix = key.slice(prefix.length);
  if (suffix === SOURCE_SNAPSHOT_SHA_SUFFIX)
    return true;
  if ([
    "published",
    "published_at",
...
function canonicalizeManagedAuthorialFields(frontmatter, prefix) {
  const key = (suffix) => `${prefix}${suffix}`;
  const value = (suffix) => frontmatter[key(suffix)];
  ...
  frontmatter[key("post_access")] = ["public", "members", "paid"].includes(rawAccess) ? rawAccess : "paid";
  ...
  frontmatter[key("tags")] = Array.isArray(tags) ? tags.filter((tag) => typeof tag === "string").flatMap((tag) => tag.split(",").map((part) => part.trim()).filter(Boolean)) : [];
}
...
var MAX_ENTRY_COUNT = 512;
var MAX_ENTRY_BYTES = 64 * 1024 * 1024;
var MAX_TOTAL_UNCOMPRESSED_BYTES = 256 * 1024 * 1024;
async function inflateRaw(data, expectedSize) {
  ...
  if (total !== expectedSize)
    throw new Error("Textpack zip entry size does not match its directory record");
...
async function readZip(buf) {
  if (buf.byteLength > MAX_ARCHIVE_BYTES)
    throw new Error("Textpack archive is too large");
  if (buf.byteLength < 22)
    throw new Error("Not a zip file (archive is too short)");
  ...
  if (entryCount > MAX_ENTRY_COUNT)
    throw new Error("Textpack contains too many files");
  if (cdOffset >= eocd)
  ...
  for (let i = 0; i < entryCount; i++) {
    ...
    if (entryNames.has(name))
      throw new Error(`Textpack contains a duplicate zip path: ${name}`);
    entryNames.add(name);
    if (uncompressedSize > MAX_ENTRY_BYTES)
      throw new Error(`Textpack entry is too large: ${name}`);
  ...
  for (const e of entries) {
    ...
    if (view.getUint16(lh + 8, true) !== e.method)
      throw new Error("Zip method differs between directory records");
    const nameLen = view.getUint16(lh + 26, true);
    const extraLen = view.getUint16(lh + 28, true);
    const dataStart = lh + 30 + nameLen + extraLen;
    if (dataStart + e.compressedSize > buf.byteLength)
      throw new Error("Corrupt zip entry data range");
    const localName = new TextDecoder().decode(bytes.subarray(lh + 30, lh + 30 + nameLen));
    if (localName !== e.name)
      throw new Error("Zip path differs between directory records");
    const raw = bytes.subarray(dataStart, dataStart + e.compressedSize);
    if (e.method === 0) {
      if (raw.byteLength !== e.uncompressedSize)
        throw new Error(`Stored zip entry has the wrong size: ${e.name}`);
...
async function parseTextpack(buf, fallbackName) {
  ...
  for (const [path, data] of files) {
    ...
    if ((_a = parts[0]) == null ? void 0 : _a.endsWith(".textbundle")) {
      if (sawRootlessContent)
        throw new Error("Textpack mixes rooted and rootless content");
      if (bundleRoot && bundleRoot !== parts[0])
        throw new Error("Textpack contains multiple bundle roots");
      bundleRoot = parts[0];
    ...
    if (/^text\.(markdown|md|txt)$/i.test(rel)) {
      if (markdown !== null)
        throw new Error("Textpack contains more than one Markdown document");
      markdown = new TextDecoder().decode(data);
    } else if (rel === "info.json") {
      if (sawInfo)
        throw new Error("Textpack contains more than one info.json");
      sawInfo = true;
      try {
      ...
      if (assets.has(base))
        throw new Error(`Textpack contains duplicate asset name: ${base}`);
  ...
  if (declaredProvenance) {
    ...
    const hasDeclaredCommit = declaredProvenance.gitCommit !== void 0 && declaredProvenance.gitCommit !== null;
    const declaredCommit = normalizeFullGitCommit(declaredProvenance.gitCommit);
    if (declaredProvenance.schema !== TEXTPACK_PROVENANCE_SCHEMA || !declaredPayload || hasDeclaredCommit && !declaredCommit) {
...
// src/versioning/note-version.ts
var import_obsidian11 = require("obsidian");
var GIT_TIMEOUT_MS = 2e4;
var GIT_MAX_BUFFER = 1024 * 1024;
var repositoryTails = /* @__PURE__ */ new Map();
...
function commandDetail(result) {
  ...
  if (!raw)
    return void 0;
  return (raw.split(/\r?\n/, 1)[0] || "").replace(/\s+/g, " ").slice(0, 240) || void 0;
}
async function sourceStillMatches(app, file, expectedSource) {
  try {
    ...
    runtime.execFile(
      ...
      (error, stdout, stderr) => {
        ...
        resolve({
          ...
          errorCode: typeof error.code === "string" ? error.code : void 0,
...
function gitInRepository(context, args) {
...
async function withRepositoryLock(repositoryRoot, action) {
  var _a;
  const previous = (_a = repositoryTails.get(repositoryRoot)) != null ? _a : Promise.resolve();
  let release = () => void 0;
...
async function repositoryIsBusy(context) {
...
async function worktreeBlobMatchesHead(context, hasHead) {
  if (!hasHead)
...
async function previousNoteCommit(context, commit) {
  ...
  const previous = commits[0] === commit.toLowerCase() ? commits[1] : void 0;
  return previous && /^(?:[0-9a-f]{40}|[0-9a-f]{64})$/.test(previous) ? previous : void 0;
}
async function stagedNoteDiffersFromWorktree(context) {
  ...
  if (staged.exitCode === 0)
    return false;
  if (staged.exitCode !== 1 || staged.errorCode || staged.timedOut)
    return null;
...
async function cleanVersion(context, app, file, expectedSource) {
  ...
  return previous ? { kind: "git", commit, previousNoteCommit: previous, createdCommit: false } : { kind: "git", commit, createdCommit: false };
}
async function ensureVersionInRepository(context, app, file, expectedSource) {
  var _a;
  if (!await sourceStillMatches(app, file, expectedSource)) {
    return unavailable("source-changed");
  }
  ...
  if (busy === null)
    return unavailable("repository-busy", "Could not inspect the Git repository state.");
  if (busy)
    return unavailable("repository-busy");
  const branch = await gitInRepository(context, ["symbolic-ref", "-q", "HEAD"]);
  if (branch.exitCode !== 0)
    return unavailable("detached-head", commandDetail(branch));
  const conflicts = await gitInRepository(context, ["ls-files", "--unmerged"]);
  if (conflicts.exitCode !== 0)
    return unavailable("repository-busy", commandDetail(conflicts));
  if (conflicts.stdout.trim())
    return unavailable("conflict");
  const head = await gitInRepository(context, ["rev-parse", "--verify", "HEAD"]);
  if (head.errorCode || head.timedOut)
    return unavailable("repository-busy", commandDetail(head));
  const hasHead = head.exitCode === 0;
  const blobState = await worktreeBlobMatchesHead(context, hasHead);
  if (!blobState)
    return unavailable("verification-failed", "Could not hash the note through Git.");
  ...
  if (stagedNoteDiffers)
    return unavailable("staged-note");
  if (needsIntentToAdd) {
    ...
    if (ignored.exitCode === 0)
      return unavailable("ignored");
    if (ignored.exitCode !== 1)
      return unavailable("commit-failed", commandDetail(ignored));
  }
  ...
  if (userName.exitCode !== 0 || userEmail.exitCode !== 0 || !userName.stdout.trim() || !userEmail.stdout.trim()) {
    return unavailable("identity-missing", (_a = commandDetail(userName)) != null ? _a : commandDetail(userEmail));
  }
  if (!await sourceStillMatches(app, file, expectedSource)) {
    return unavailable("source-changed");
  }
  ...
  if (needsIntentToAdd) {
    ...
    if (addIntent.exitCode !== 0)
      return unavailable("commit-failed", commandDetail(addIntent));
    addedIntent = true;
  }
  ...
  if (committedHead.exitCode !== 0 || !commit || !(verifiedBlob == null ? void 0 : verifiedBlob.matches)) {
    return unavailable("verification-failed", commandDetail(committedHead));
  }
  const previous = await previousNoteCommit(context, commit);
  return previous ? { kind: "git", commit, previousNoteCommit: previous, createdCommit: true } : { kind: "git", commit, createdCommit: true };
}
async function ensureNoteVersioned(app, file, expectedSource) {
  if (!await sourceStillMatches(app, file, expectedSource)) {
    return unavailable("source-changed");
  }
  if (!import_obsidian11.Platform.isDesktopApp)
    return unavailable("mobile");
  if (!(app.vault.adapter instanceof import_obsidian11.FileSystemAdapter))
    return unavailable("no-filesystem");
  let runtime;
  try {
    runtime = await loadDesktopRuntime();
  } catch (error) {
    return unavailable("git-unavailable", error instanceof Error ? error.message : void 0);
  }
  let noteAbsolutePath;
  try {
    ...
    return unavailable("no-filesystem", error instanceof Error ? error.message : void 0);
  }
  ...
  if (repository.exitCode !== 0)
    return unavailable("not-repository", commandDetail(repository));
  let repositoryRoot;
  try {
    repositoryRoot = await runtime.fs.realpath(stripFinalLineEnding(repository.stdout));
  } catch (error) {
    return unavailable("not-repository", error instanceof Error ? error.message : void 0);
  }
  const relativePath = runtime.path.relative(repositoryRoot, noteAbsolutePath);
  if (!relativePath || relativePath === ".." || relativePath.startsWith(`..${runtime.path.sep}`) || runtime.path.isAbsolute(relativePath)) {
...
var GhostWriterManagerPlugin = class extends import_obsidian12.Plugin {
  ...
  async onload() {
    ...
    if (DEV_MODE) {
      ...
      this.syncDebounced = (0, import_obsidian12.debounce)(
        async (file) => {
      ...
      this.registerEvent(
        this.app.vault.on("modify", (file) => {
          if (file instanceof import_obsidian12.TFile && this.syncDebounced) {
    ...
    this.registerEvent(this.app.vault.on("rename", (af, oldPath) => this.reindexRenamed(af, (0, import_obsidian12.normalizePath)(oldPath))));
    this.registerEvent(
      this.app.vault.on("delete", (af) => {
        if (!this.settings.promptDeleteOnFolderDelete)
          return;
        if (af instanceof import_obsidian12.TFolder) {
          this.pendingDeletedFolders.push((0, import_obsidian12.normalizePath)(af.path));
    ...
    this.app.workspace.onLayoutReady(() => {
      ...
      this.registerEvent(this.app.vault.on("create", (af) => {
        if (!this.settings.autoImportTextpacks)
          return;
        if (af instanceof import_obsidian12.TFile && af.extension === "textpack") {
          window.setTimeout(() => {
    ...
    this.addCommand({
      ...
      callback: () => {
        ...
        const cache = this.app.metadataCache.getFileCache(file);
        if (!(cache == null ? void 0 : cache.frontmatter)) {
          new import_obsidian12.Notice("No frontmatter found");
        ...
        if (ghostKeys.length === 0) {
          new import_obsidian12.Notice(`No properties with prefix "${this.settings.yamlPrefix}" found`);
          console.debug("[Ghost Debug] Available keys:", Object.keys(cache.frontmatter));
        } else {
          new import_obsidian12.Notice(`Found ${ghostKeys.length} Ghost properties`);
    ...
    this.addCommand({
      ...
      callback: async () => {
        ...
        if (!url || !apiKey) {
          new import_obsidian12.Notice("Please configure a blog URL and admin API key first");
          return;
        }
        try {
          ...
          if (result) {
            new import_obsidian12.Notice("JWT generation successful! Connection works.");
            console.debug("[Ghost Debug] JWT and connection working");
          } else {
            new import_obsidian12.Notice("Connection failed - check console for details");
    ...
    this.addCommand({
      ...
      callback: () => {
        ...
        void this.app.vault.read(file).then((content) => {
          ...
          new import_obsidian12.Notice("File data logged to console");
        }).catch((error) => {
          console.error("[Ghost Debug] Error reading file:", error);
          new import_obsidian12.Notice(`Error reading file: ${error.message}`);
        });
      }
    });
    this.addCommand({
  ...
  async syncBlogFolder(blog) {
    const folder = (0, import_obsidian12.normalizePath)(blog.folder);
  ...
  imageCachePath() {
    var _a;
    return (0, import_obsidian12.normalizePath)(`${(_a = this.manifest.dir) != null ? _a : "."}/image-cache.json`);
  }
  ...
  loadApiKey() {
    ...
    try {
      if (!this.app.secretStorage) {
        ...
        new import_obsidian12.Notice("Obsidian secrets API not available. Please update Obsidian to the latest version.");
        return "";
      }
      ...
      if (!apiKey) {
        ...
        new import_obsidian12.Notice(`Secret "${this.settings.ghostApiKeySecretName}" not found in Keychain. Please create it in settings \u2192 Keychain.`);
        return "";
      }
      ...
      new import_obsidian12.Notice(`Error loading secret: ${error.message}`);
      return "";
    }
  }
  async testGhostConnection() {
    ...
    try {
      ...
      if (title) {
        new import_obsidian12.Notice(`Successfully connected to ${title}`);
      } else {
        new import_obsidian12.Notice("Failed to connect to ghost. Please check your credentials.");
      }
    } catch (error) {
      console.error("Ghost connection test failed:", error);
      new import_obsidian12.Notice(`Connection failed: ${error.message}`);
    }
  }
  async syncWithGhost() {
    ...
    try {
      ...
      new import_obsidian12.Notice(`Sync failed: ${error.message}`);
    }
  }
  async createNewGhostPost() {
    var _a;
    try {
      const syncFolderPath = (0, import_obsidian12.normalizePath)(((_a = this.defaultBlog()) == null ? void 0 : _a.folder) || this.settings.syncFolder);
      ...
      const filePath = (0, import_obsidian12.normalizePath)(`${syncFolderPath}/${fileName}`);
      const content = generateNewPostTemplate(this.settings);
      const file = await this.app.vault.create(filePath, content);
      const leaf = this.app.workspace.getLeaf(false);
      await leaf.openFile(file);
      new import_obsidian12.Notice("New ghost post created!");
    } catch (error) {
      console.error("Error creating new Ghost post:", error);
      new import_obsidian12.Notice(`Failed to create new post: ${error.message}`);
    }
  }
  /**
  ...
  openImportFromGhostModal() {
    ...
    if (!hasBlogKey && !hasGlobal) {
      new import_obsidian12.Notice("Configure a blog (URL + admin API key) in settings first");
      return;
    }
  ...
  async importPostAsNote(post, ghostUrl, blog) {
    ...
    try {
      const prefix = this.settings.yamlPrefix;
      const folderPath = (0, import_obsidian12.normalizePath)(blog ? blog.folder : this.settings.syncFolder);
      ...
      const filePath = (0, import_obsidian12.normalizePath)(`${folderPath}/${safeFileName}.md`);
      const bodyMarkdown = htmlToMarkdown(stripRenderedPublicationProvenanceHtml((_f = post.html) != null ? _f : ""));
      const content = `---
${(0, import_obsidian12.stringifyYaml)(fm)}---
...
${bodyMarkdown}`;
      ...
      if (this.app.vault.getAbstractFileByPath(filePath)) {
        const ts = Date.now();
        finalPath = (0, import_obsidian12.normalizePath)(`${folderPath}/${safeFileName}-${ts}.md`);
      }
      const file = await this.app.vault.create(finalPath, content);
      const leaf = this.app.workspace.getLeaf(false);
      await leaf.openFile(file);
      new import_obsidian12.Notice(`Imported: "${title}"${blog ? ` (${blog.name})` : ""}`);
    } catch (error) {
      console.error("[Ghost Import] Error importing post:", error);
      new import_obsidian12.Notice(`Failed to import post: ${error.message}`);
    }
  }
  /**
  ...
  async seedActiveNoteFromGhost(file) {
    ...
    if (!(metadata == null ? void 0 : metadata.slug)) {
      new import_obsidian12.Notice(`Set ${prefix}slug on this note first to seed from Ghost`);
      return;
    }
    const blog = this.resolveBlogsForFile(file)[0];
    if (!blog || !this.loadApiKeyForSecret(blog.apiKeySecretName).trim()) {
      new import_obsidian12.Notice("This note's blog has no API key \u2014 set it in settings.");
      return;
    }
    if (this.readBlogId(fmObj, blog)) {
      new import_obsidian12.Notice(`This note is already linked to a post on ${blog.name}; nothing to seed.`);
      return;
    }
    this.syncEngine.setActiveBlog(this.getClientForBlog(blog), blog.url, blog.folder, false, void 0, blog.name);
    try {
      new import_obsidian12.Notice(`Looking up "${metadata.slug}" on ${blog.name}\u2026`);
      await this.syncEngine.seedNoteFromGhostBySlug(file, metadata.slug);
    } catch (error) {
      new import_obsidian12.Notice(`Seed failed: ${error.message}`);
    } finally {
      this.restoreDefaultBlogContext();
    }
  }
  ...
  ghostPostsRoot() {
    return (0, import_obsidian12.normalizePath)((this.settings.syncFolder || "Ghost Posts").trim() || "Ghost Posts");
  }
  ...
  defaultFolderFor(blog) {
    ...
    return host ? (0, import_obsidian12.normalizePath)(`${root}/${host}`) : root;
  }
  /** True if any markdown file lives in this folder (or below). */
  folderHasNotes(folder) {
    const f = (0, import_obsidian12.normalizePath)(folder || "");
    if (!f || f === "/")
      return false;
  ...
  blogForPath(path) {
    ...
    let bestLen = -1;
    for (const b of this.settings.blogs) {
      const f = (0, import_obsidian12.normalizePath)(b.folder);
  ...
  resolveBlogsForFile(file) {
    ...
    if (inferred && (0, import_obsidian12.normalizePath)(inferred.folder) !== this.ghostPostsRoot())
      return [inferred];
  ...
  async organizeBlogFolders() {
    ...
    for (const blog of this.settings.blogs) {
      if (!this.hostOf(blog.url))
        continue;
      const from = (0, import_obsidian12.normalizePath)(blog.folder);
    ...
    for (const { blog, from, to } of plans) {
      if (from !== to) {
        for (const [path, ownerId] of owners) {
          ...
          try {
            if (await this.moveNoteTo(file, (0, import_obsidian12.normalizePath)(`${to}/${rel}`)))
              moved++;
          } catch (e) {
        ...
        if (fromCounts.get(from) === 1) {
          ...
          for (const f of this.app.vault.getMarkdownFiles().filter((x) => x.path.startsWith(archPrefix))) {
            try {
              if (await this.moveNoteTo(f, (0, import_obsidian12.normalizePath)(`${to}/${this.archiveName()}/${f.path.slice(archPrefix.length)}`)))
                moved++;
            } catch (e) {
  ...
  async inheritedTextpackNoteVersion(file, content, frontmatter, prefix) {
    var _a, _b;
    const result = await validateInheritedTextpackSource(
      ...
      async (relativePath) => {
        var _a2, _b2;
        const parent2 = (_b2 = (_a2 = file.parent) == null ? void 0 : _a2.path) != null ? _b2 : "";
        const path = (0, import_obsidian12.normalizePath)(parent2 ? `${parent2}/${relativePath}` : relativePath);
    ...
    if (result.kind !== "valid" || !result.source.gitCommit)
      return void 0;
    ...
    return {
      ...
      assetSha256ByVaultPath: new Map(result.source.assets.map((asset) => [
        (0, import_obsidian12.normalizePath)(parent ? `${parent}/${asset.name}` : asset.name),
        asset.sha256
      ]))
    };
  }
  ...
  async syncFileToBlogs(file, blogs) {
    ...
    let fmObj = (_c = (_b = this.app.metadataCache.getFileCache(file)) == null ? void 0 : _b.frontmatter) != null ? _c : {};
    const split0 = splitFrontmatter(content0);
    if (split0) {
      try {
        const d = (0, import_obsidian12.parseYaml)(split0.raw);
        if (d && typeof d === "object")
          fmObj = d;
      } catch (e) {
        console.debug("[Ghost Sync] Cannot sync note with invalid frontmatter:", file.path, e);
        new import_obsidian12.Notice(`Cannot sync "${file.basename}": frontmatter YAML is invalid. Fix the red properties and try again.`);
        return false;
      }
    }
    const fmStr = (k) => typeof fmObj[k] === "string" ? String(fmObj[k]) : "";
    const noteMetadata = parseGhostMetadata(fmObj, prefix);
    const hasPublishTarget = blogs.some(
      (blog) => !!blog.url.trim() && !!this.loadApiKeyForSecret(blog.apiKeySecretName).trim()
    );
    let noteVersion;
    let inheritedAssetSha256;
    if ((noteMetadata == null ? void 0 : noteMetadata.published) && !noteMetadata.no_sync && hasPublishTarget) {
      ...
      if (noteVersion.kind === "unavailable") {
        console.debug(`[Ghost Version] Using publication SHA-256 for ${file.path}: ${noteVersion.reason}`, (_d = noteVersion.detail) != null ? _d : "");
      }
    }
    if (await this.app.vault.read(file) !== content0) {
      new import_obsidian12.Notice(`Cannot sync "${file.basename}": the note changed while preparing publication. Try again.`);
      return false;
    }
    ...
    for (const blog of blogs) {
      ...
      if (!this.loadApiKeyForSecret(blog.apiKeySecretName).trim()) {
        new import_obsidian12.Notice(`Blog "${blog.name}" has no API key \u2014 set it in settings.`);
        ok = false;
        continue;
      }
      ...
      try {
        ...
        new import_obsidian12.Notice(`Sync to ${blog.name} failed: ${e.message}`);
        ok = false;
      }
  ...
  async syncAllRouted() {
    ...
    if (files.size === 0) {
      new import_obsidian12.Notice("No notes to sync (check your blog folders).");
      return;
    }
    new import_obsidian12.Notice(`Syncing ${files.size} note(s)\u2026`);
    let success = 0;
    let failed = 0;
    for (const f of files) {
    ...
    new import_obsidian12.Notice(`Sync complete: ${success} ok${failed ? `, ${failed} failed` : ""}`);
  }
  ...
  openSetBlogsModal(file) {
    ...
    new SelectBlogsModal(
      ...
      async (chosen) => {
        ...
        await this.saveSettings();
        new import_obsidian12.Notice(`Note will publish to: ${chosen.map((b) => b.name).join(", ")}`);
      }
    ).open();
  }
  ...
  async importTextpack(pack, blog, titleOptions) {
    ...
    let assetDir;
    while (true) {
      notePath = (0, import_obsidian12.normalizePath)(`${folder}/${fileName}${importSuffix}.md`);
      assetFolderName = `${slug}${importSuffix}`;
      assetDir = (0, import_obsidian12.normalizePath)(`${folder}/assets/${assetFolderName}`);
      ...
      if (!noteCollision && !assetCollision)
        break;
      collisionNumber++;
  ...
  async importVaultTextpack(file) {
    ...
    try {
      ...
      new import_obsidian12.Notice(`Textpack import failed for ${file.name}: ${e.message}`);
      return false;
    }
  }
  ...
  async importVaultTextpacks(notifyWhenNone) {
    ...
    if (packs.length === 0) {
      if (notifyWhenNone)
        new import_obsidian12.Notice("No .textpack files found in the vault.");
      return;
    }
  ...
  async importAllFromBlog(blog) {
    ...
    if (!blog.url || !apiKey) {
      new import_obsidian12.Notice(`Blog "${blog.name}" is missing its URL or API key.`);
      return;
    }
    const client = this.getClientForBlog(blog);
    try {
      new import_obsidian12.Notice(`Importing all posts from ${blog.name}\u2026`);
      ...
      new import_obsidian12.Notice(`Imported ${count} post${count === 1 ? "" : "s"} from ${blog.name} into ${folder}`);
    } catch (e) {
      new import_obsidian12.Notice(`Import from ${blog.name} failed: ${e.message}`);
    }
  }
  ...
  async writePostAsNoteInFolder(post, editorUrl, folder, blog) {
    ...
    const body = htmlToMarkdown(stripRenderedPublicationProvenanceHtml((_f = post.html) != null ? _f : ""));
    const content = `---
${(0, import_obsidian12.stringifyYaml)(fm)}---
...
${body}`;
    let path = (0, import_obsidian12.normalizePath)(`${folder}/${safe}.md`);
    if (this.app.vault.getAbstractFileByPath(path)) {
      path = (0, import_obsidian12.normalizePath)(`${folder}/${safe}-${post.id}.md`);
    }
    if (this.app.vault.getAbstractFileByPath(path))
      return;
  ...
  async clearImageCache() {
    ...
    this.imageCache = {};
    await this.saveImageCache();
    new import_obsidian12.Notice(`Cleared ghost image cache (${n} ${n === 1 ? "entry" : "entries"})`);
  }
  /**
  ...
  async migratePrefix(newPrefix) {
    ...
    if (!newPrefix || newPrefix === oldPrefix)
      return;
    new import_obsidian12.Notice("Migrating ghost property prefix\u2026");
    ...
    await this.saveSettings();
    new import_obsidian12.Notice(`Migrated ${count} note${count === 1 ? "" : "s"} from "${oldPrefix}" to "${newPrefix}"`);
  }
  /**
  ...
  async openEditPropertiesModal(file) {
    ...
    if (split) {
      try {
        const d = (0, import_obsidian12.parseYaml)(split.raw);
        if (d && typeof d === "object")
          fmObj = d;
      } catch (e) {
      }
    ...
    new EditGhostPropertiesModal(this.app, file.basename, initial, info, availableBlogs, async (form, doSync) => {
      ...
      new import_obsidian12.Notice("Ghost properties saved");
      if (doSync) {
        try {
          const synced = await this.syncFileToBlogs(file, selectedBlogs.length ? selectedBlogs : this.resolveBlogsForFile(file));
          if (!synced) {
            new import_obsidian12.Notice("Ghost properties saved, but sync did not complete. Check the note properties and blog settings.");
          }
        } catch (e) {
          new import_obsidian12.Notice(`Sync failed: ${e.message}`);
        }
      }
      ...
      if (freshSplit) {
        try {
          const d = (0, import_obsidian12.parseYaml)(freshSplit.raw);
          if (d && typeof d === "object")
            freshFm = d;
        } catch (e) {
        }
...
${bodyMarkdown}`;
        ...
        new import_obsidian12.Notice(`Linked and updated note from Ghost: "${title}"${blog ? ` (${blog.name})` : ""}`);
      } else {
        ...
        new import_obsidian12.Notice(`Linked and synced note to Ghost: "${file.basename}"${blog ? ` (${blog.name})` : ""}`);
      }
    } catch (error) {
      console.error("[Ghost Link] Error linking note:", error);
      new import_obsidian12.Notice(`Failed to link note: ${error.message}`);
    }
  }
  ...
  async ensureInFolder(file, folderPath) {
    var _a, _b;
    const dest = (0, import_obsidian12.normalizePath)(folderPath);
    ...
    const newPath = (0, import_obsidian12.normalizePath)(`${dest}/${file.name}`);
  ...
  async syncCurrentNote(file) {
    ...
    if (!hasGhostProps) {
      ...
      new import_obsidian12.Notice("Ghost properties added. Syncing\u2026");
    ...
    if (((_a = cache == null ? void 0 : cache.frontmatter) == null ? void 0 : _a[noSyncKey]) === true || ((_b = cache == null ? void 0 : cache.frontmatter) == null ? void 0 : _b[noSyncKey]) === "true") {
      new import_obsidian12.Notice("Sync is disabled for this note (no_sync: true).");
      return;
    }
    let targetFile = file;
    if (!this.fileInAnyBlogFolder(file)) {
      const blog = this.resolveBlogsForFile(file)[0];
      const folder = (0, import_obsidian12.normalizePath)((blog ? blog.folder : "") || this.settings.syncFolder);
      new import_obsidian12.Notice(`Moving note to ${folder}`);
      const newPath = await this.moveNoteTo(file, (0, import_obsidian12.normalizePath)(`${folder}/${file.name}`));
      const movedFile = newPath ? this.app.vault.getFileByPath(newPath) : null;
      if (!movedFile) {
        new import_obsidian12.Notice("Failed to move file to the blog folder.");
        return;
      }
      targetFile = movedFile;
    }
    new import_obsidian12.Notice(`Syncing "${targetFile.basename}"\u2026`);
    const success = await this.syncFileRouted(targetFile, true);
    if (!success) {
      new import_obsidian12.Notice(`Sync failed for "${targetFile.basename}". Check the console for details.`);
    }
  }
  async addGhostPropertiesToCurrentNote(file) {
    ...
    try {
      ...
      if (addGhostPropertiesToContent(before, this.settings) === before) {
        new import_obsidian12.Notice("This note already has all ghost properties");
        return;
      }
      await this.app.vault.process(file, (content) => addGhostPropertiesToContent(content, this.settings));
      new import_obsidian12.Notice("Ghost properties added! This note will now sync with ghost.");
      if (!this.fileInAnyBlogFolder(file)) {
        const blog = this.resolveBlogsForFile(file)[0];
        const folder = (0, import_obsidian12.normalizePath)((blog ? blog.folder : "") || this.settings.syncFolder);
        const newPath = await this.moveNoteTo(file, (0, import_obsidian12.normalizePath)(`${folder}/${file.name}`));
        if (newPath)
          new import_obsidian12.Notice(`File moved to ${folder}`);
      }
    } catch (error) {
      console.error("Error adding Ghost properties:", error);
      new import_obsidian12.Notice(`Failed to add Ghost properties: ${error.message}`);
    }
  }
  async scheduleCurrentNote(file) {
    ...
    try {
      ...
      new import_obsidian12.Notice(`Failed to fetch posts from ${blog.name}: ${error.message}`);
      return;
    }
    ...
    const applyDate = async () => {
      ...
      new import_obsidian12.Notice(`Scheduled for ${newDateLabel}`);
    };
    if (existingDate) {
  ...
  isArchivePath(path) {
    ...
    const folders = this.settings.blogs.map((b) => (0, import_obsidian12.normalizePath)(b.folder));
    folders.push((0, import_obsidian12.normalizePath)(this.settings.syncFolder));
  ...
  archiveTargetFor(notePath) {
    ...
    return (0, import_obsidian12.normalizePath)(`${base}/${name}/${rel}`);
  }
  ...
  async archiveNote(file) {
    ...
    if (moved instanceof import_obsidian12.TFile) {
      try {
        const prefix = this.settings.yamlPrefix;
  ...
  reindexRenamed(af, oldPath) {
    ...
    if (af instanceof import_obsidian12.TFolder) {
      ...
      for (const [path, entries2] of [...this.ghostIndex]) {
        ...
        const np = (0, import_obsidian12.normalizePath)(`${af.path}/${path.slice(prefix.length)}`);
    ...
    if (af instanceof import_obsidian12.TFile && af.extension === "md" && this.fileInAnyBlogFolder(af)) {
  ...
  openBulkDeleteCommand() {
    if (this.settings.blogs.length === 0) {
      new import_obsidian12.Notice("No blogs configured.");
      return;
    }
    const def = this.defaultBlog();
    new SelectBlogsModal(
      ...
      (chosen) => {
        ...
        const folders = chosen.map((b) => (0, import_obsidian12.normalizePath)(b.folder));
        const items = [];
        for (const f of this.app.vault.getMarkdownFiles()) {
    ...
    new OrphanPostModal(this.app, job.blog.name, url, (decision) => {
      void (async () => {
        try {
          ...
          new import_obsidian12.Notice(`Failed: ${e.message}`);
        }
  ...
  async deleteOrphanPost(file, blog, ghostId) {
    ...
    new import_obsidian12.Notice(`Deleted orphaned post on ${blog.name}`);
  }
  ...
  async keepOrphanInBoth(file, blog) {
    ...
    new import_obsidian12.Notice(`Kept "${blog.name}" \u2014 this note now publishes to both`);
  }
  ...
  async normalizeBlogReferences() {
    ...
    for (const file of files) {
      try {
        ...
        let fmObj;
        try {
          fmObj = (0, import_obsidian12.parseYaml)(split.raw) || {};
        } catch (e) {
          continue;
        }
    ...
    new import_obsidian12.Notice(`Normalized blog references in ${changed} note${changed === 1 ? "" : "s"}`);
  }
  ...
  async migrateBlogRename(oldName, blog) {
    ...
    if (changed)
      new import_obsidian12.Notice(`Updated ${changed} note${changed === 1 ? "" : "s"} for renamed blog "${blog.name}"`);
  }
};
var ConfirmScheduleModal = class extends import_obsidian12.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian12.ButtonComponent(buttonRow).setButtonText("Overwrite").setCta().onClick(() => {
      this.onConfirm();
      this.close();
    });
    new import_obsidian12.ButtonComponent(buttonRow).setButtonText("Cancel").onClick(() => this.close());
  }
  onClose() {
    this.contentEl.empty();
  }
...
var DeleteConfirmModal = class extends import_obsidian12.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian12.ButtonComponent(row).setButtonText("Delete").setWarning().onClick(() => this.finish("delete"));
    new import_obsidian12.ButtonComponent(row).setButtonText("Skip").onClick(() => this.finish("skip"));
    new import_obsidian12.ButtonComponent(row).setButtonText("Stop \u2014 cancel all remaining").onClick(() => this.finish("stop"));
  }
  finish(decision) {
    this.decided = true;
...
var SimpleConfirmModal = class extends import_obsidian12.Modal {
  ...
  onOpen() {
    ...
    const row = contentEl.createDiv({ cls: "modal-button-container" });
    new import_obsidian12.ButtonComponent(row).setButtonText(this.confirmLabel).setWarning().onClick(() => this.finish(true));
    new import_obsidian12.ButtonComponent(row).setButtonText("Cancel").onClick(() => this.finish(false));
  }
  finish(v) {
    this.decided = true;
    this.onResult(v);
...
var ImportTextpackModal = class extends import_obsidian12.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian12.Setting(contentEl).setName("Import into blog").addDropdown((d) => {
    ...
    new import_obsidian12.Setting(contentEl).setName("Primary title on import").setDesc("Choose which source becomes the publishing title. Imported notes collapse a leading heading when it duplicates the chosen title.").addDropdown((d) => {
    ...
    new import_obsidian12.Setting(contentEl).setName("Update secondary title").setDesc("When the chosen source differs, rewrite the other title slot to match before collapsing duplicate headings.").addToggle((t) => t.setValue(this.updateSecondaryTitle).onChange((value) => {
    ...
    const row = contentEl.createDiv({ cls: "modal-button-container" });
    new import_obsidian12.ButtonComponent(row).setButtonText("Import").setCta().onClick(() => {
      void (async () => {
        var _a;
        if (!this.parsed) {
        ...
        if (!blog) {
          new import_obsidian12.Notice("No blog configured \u2014 add one in settings.");
          return;
        }
        this.close();
        try {
          ...
          new import_obsidian12.Notice(`Import failed: ${e.message}`);
        }
      })();
    });
...
var OrphanPostModal = class extends import_obsidian12.Modal {
  ...
  onOpen() {
    ...
    const row = contentEl.createDiv({ cls: "modal-button-container" });
    new import_obsidian12.ButtonComponent(row).setButtonText("Delete on ghost").setWarning().onClick(() => this.finish("delete"));
    new import_obsidian12.ButtonComponent(row).setButtonText("Keep in both").onClick(() => this.finish("keep"));
    new import_obsidian12.ButtonComponent(row).setButtonText("Decide later").onClick(() => this.finish("later"));
  }
  finish(decision) {
    this.decided = true;
...
var BulkDeleteModal = class extends import_obsidian12.Modal {
...
var GhostWriterSettingTab = class extends import_obsidian12.PluginSettingTab {
  constructor(app, plugin) {
    super(app, plugin);
    this.plugin = plugin;
  }
  ...
  renderBlogsSettings(containerEl) {
    ...
    new import_obsidian12.Setting(containerEl).setHeading().setName("Ghost blogs");
    ...
    if (colliding.length >= 2) {
      ...
      (0, import_obsidian12.setIcon)(warnEl.createSpan({ cls: "omnighost-status-icon" }), "alert-triangle");
    ...
    plugin.settings.blogs.forEach((blog) => {
      ...
      new import_obsidian12.Setting(containerEl).setName("Site address").addText((t) => {
        ...
        t.inputEl.addEventListener("blur", () => {
          void (async () => {
            ...
            if (blog.folderAuto !== false) {
              ...
              const cur = (0, import_obsidian12.normalizePath)(blog.folder || "");
              if (cur === plugin.ghostPostsRoot() || !plugin.folderHasNotes(cur)) {
                blog.folder = derived;
              } else {
                new import_obsidian12.Notice(`"${blog.name}": folder kept at ${cur} \u2014 run "Organize folders by domain" to move its notes to ${derived}`);
              }
              if (folderInput)
      ...
      keySetting.addText((t) => {
        ...
        if (!pendingKey) {
          new import_obsidian12.Notice("Enter a key first");
          return;
        }
        ...
        try {
          ...
          if (title) {
            ...
            new import_obsidian12.Notice(`${blog.name || "Blog"}: key saved but connection failed`);
          }
        } catch (e) {
          b.setDisabled(false);
          new import_obsidian12.Notice(`Could not save key: ${e.message}`);
        }
      }));
      new import_obsidian12.Setting(containerEl).setName("Key secret name").setDesc("Keychain secret holding this key (managed automatically; one per blog)").addText((t) => {
      ...
      new import_obsidian12.Setting(containerEl).setName("Folder").setDesc("Vault folder for this blog's posts. Leave blank to derive it from the site address (root/domain).").addText((t) => {
        ...
        t.setPlaceholder(plugin.defaultFolderFor(blog)).setValue(blog.folderAuto !== false ? "" : blog.folder).onChange(async (v) => {
          const val = v.trim();
          if (val) {
            blog.folder = (0, import_obsidian12.normalizePath)(val);
      ...
      new import_obsidian12.Setting(containerEl).setName("Auto-sync this folder").setDesc("When off, this folder is never auto-synced (manual sync still works).").addToggle((t) => t.setValue(blog.syncEnabled !== false).onChange(async (v) => {
        blog.syncEnabled = v;
        await plugin.saveSettings();
      }));
      new import_obsidian12.Setting(containerEl).setName("Sync interval (minutes)").setDesc("How often to auto-sync this folder. 0 = off. Leave blank to use the global interval.").addText((t) => t.setPlaceholder(`global (${plugin.settings.syncInterval})`).setValue(blog.syncIntervalMinutes == null ? "" : String(blog.syncIntervalMinutes)).onChange(async (v) => {
        const s = v.trim();
        if (s === "") {
      ...
      new import_obsidian12.Setting(containerEl).setName("Test connection").addButton((btn) => btn.setButtonText("Test").onClick(async () => {
        const title = await plugin.getClientForBlog(blog).testConnection();
        new import_obsidian12.Notice(title ? `Connected to ${title}` : `Failed to connect to ${blog.name}`);
      }));
    });
  ...
  display() {
    ...
    new import_obsidian12.Setting(containerEl).setHeading().setName("Sync settings");
    new import_obsidian12.Setting(containerEl).setName("Ghost posts root folder").setDesc('Folder that blog folders nest under: each blog with an automatic folder lives in root/domain (e.g. "Ghost Posts/chief.sc").').addText((text) => text.setPlaceholder("Ghost Posts").setValue(this.plugin.settings.syncFolder).onChange(async (value) => {
      const oldRoot = this.plugin.ghostPostsRoot();
      this.plugin.settings.syncFolder = value.trim() || "Ghost Posts";
      for (const b of this.plugin.settings.blogs) {
        if (b.folderAuto === false)
          continue;
        const cur = (0, import_obsidian12.normalizePath)(b.folder || "");
    ...
    new import_obsidian12.Setting(containerEl).setName("Sync interval").setDesc("How often to check for changes (in minutes)").addText((text) => text.setPlaceholder("15").setValue(String(this.plugin.settings.syncInterval)).onChange(async (value) => {
    ...
    new import_obsidian12.Setting(containerEl).setName("Auto-import textpacks").setDesc("When a .textpack file appears in the vault \u2014 for instance saved there from your phone \u2014 import it as a blog note and move the pack to trash.").addToggle((t) => t.setValue(this.plugin.settings.autoImportTextpacks).onChange(async (value) => {
      this.plugin.settings.autoImportTextpacks = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian12.Setting(containerEl).setName("Sync title source").setDesc("Choose which note title source is sent to ghost when syncing.").addDropdown((d) => d.addOption("metadata", "Metadata title property").addOption("heading", "First heading").setValue(this.plugin.settings.syncTitleSource).onChange(async (value) => {
      this.plugin.settings.syncTitleSource = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian12.Setting(containerEl).setName("Update secondary title on sync").setDesc("When syncing, rewrite the non-primary title slot to match the source sent to ghost.").addToggle((t) => t.setValue(this.plugin.settings.syncUpdateSecondaryTitle).onChange(async (value) => {
      this.plugin.settings.syncUpdateSecondaryTitle = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian12.Setting(containerEl).setName("Publication provenance").setDesc("Choose what readers see at the end of posts. Version metadata is always stored invisibly in each post.").addDropdown((dropdown) => dropdown.addOption("visible-hash", "Visible version and credit").addOption("visible-credit", "Visible credit only").addOption("hidden", "Hidden provenance").setValue(this.plugin.settings.publicationProvenanceVisibility).onChange(async (value) => {
      this.plugin.settings.publicationProvenanceVisibility = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian12.Setting(containerEl).setName("Verify published content directly").setDesc("Compare all managed post fields even when the stored publication hash matches. Keep this on to detect edits made directly in the publishing site.").addToggle((toggle) => toggle.setValue(this.plugin.settings.verifyGhostContentOnSync).onChange(async (value) => {
      this.plugin.settings.verifyGhostContentOnSync = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian12.Setting(containerEl).setName("YAML prefix").setDesc('Prefix for ghost metadata in frontmatter (e.g., "ghost_" will create ghost_status, ghost_tags)').addText((text) => text.setPlaceholder("Prefix used in YAML keys").setValue(this.plugin.settings.yamlPrefix).onChange(async (value) => {
      this.plugin.settings.yamlPrefix = value.trim();
      await this.plugin.saveSettings();
    }));
    new import_obsidian12.Setting(containerEl).setName("Show sync notifications").setDesc("Show notification popups when syncing files (status bar always shows sync status)").addToggle((toggle) => toggle.setValue(this.plugin.settings.showSyncNotifications).onChange(async (value) => {
      this.plugin.settings.showSyncNotifications = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian12.Setting(containerEl).setHeading().setName("Deletion");
    new import_obsidian12.Setting(containerEl).setDesc("Deletion is never automatic. Run the command \u201Cbulk delete posts (local notes + ghost)\u201D to pick a blog, review a checklist of its linked posts, and confirm.");
    new import_obsidian12.Setting(containerEl).setName("Prompt on folder delete").setDesc("When you delete a folder of synced notes, pop up the same bulk-delete checklist for their ghost posts (the local notes are already gone). Nothing is deleted until you confirm. Turn off to ignore folder deletes entirely.").addToggle((toggle) => toggle.setValue(this.plugin.settings.promptDeleteOnFolderDelete).onChange(async (value) => {
      this.plugin.settings.promptDeleteOnFolderDelete = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian12.Setting(containerEl).setName("Confirm each remote delete").setDesc("During a bulk delete, also show a per-post confirmation (post + blog name) with delete / skip / stop. The final bulk confirmation always shows regardless.").addToggle((toggle) => toggle.setValue(this.plugin.settings.confirmEachRemoteDelete).onChange(async (value) => {
      this.plugin.settings.confirmEachRemoteDelete = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian12.Setting(containerEl).setName("Archive deleted notes").setDesc("When a bulk delete removes a local note, move it into an archive subfolder of its blog folder instead of trashing it (the ghost post is still deleted). Archived notes are never re-synced.").addToggle((toggle) => toggle.setValue(this.plugin.settings.archiveDeletedNotes).onChange(async (value) => {
      this.plugin.settings.archiveDeletedNotes = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian12.Setting(containerEl).setName("Archive subfolder name").setDesc("Name of the archive subfolder created inside each blog folder (default: archive).").addText((text) => text.setPlaceholder("Archive").setValue(this.plugin.settings.archiveFolderName).onChange(async (value) => {
      this.plugin.settings.archiveFolderName = value.trim() || "Archive";
      await this.plugin.saveSettings();
    }));
    new import_obsidian12.Setting(containerEl).setHeading().setName("Scheduling");
    new import_obsidian12.Setting(containerEl).setName("Interval between posts").setDesc("Number of days between scheduled publications").addText((text) => text.setPlaceholder("7").setValue(String(this.plugin.settings.schedulingIntervalDays)).onChange(async (value) => {
    ...
    new import_obsidian12.Setting(containerEl).setName("Default publish time").setDesc("Time of day for scheduled posts (e.g. 09:00).").addText((text) => text.setPlaceholder("09:00").setValue(this.plugin.settings.defaultPublishTime).onChange(async (value) => {
